...
normalized_res=[]
for s in range(len(Presto_results)):
        normalized_res.append(round(Presto_results[0]/Presto_results[s],3))
...
plt.plot( 'x', 'y', '.-', data=df1, color='deepskyblue',linewidth=2, markersize=10, label='Presto')
...
plt.savefig('Presto_cores_scalability.pdf',bbox_inches='tight',dpi=6000, backend='pgf')
