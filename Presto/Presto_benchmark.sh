...
#%    -m, --mode                    Test mode: WIM, WIN, WIC or SPMWIN. Default mode is WIN.
...
#%                                  Default: ./results/${MODE}_${POLICY}_SF:${SCALE_FACTOR}_Workers:${TOTAL_WORKERS}_Cores_per_Workers:${CORES_PER_WORKER}_%Y%m%d%H%M%S
...
create_workers_directory() {
	for ((i = 2; i < $TOTAL_WORKERS; i += 1)) ; do
...
remove_workers_directory() {
	for ((i = 2; i < $TOTAL_WORKERS; i += 1)) ; do
		exec_cmd "rm -rf presto-worker-${i};"
	done
...
set_worker_config_files() {
	for ((i = 1; i < $TOTAL_WORKERS; i += 1)) ; do
		...
		echo "discovery.uri=http://127.0.0.1:$COORD_PORT" >> "${SCRIPT_DIR}/presto-worker-$i/etc/config.properties"

		#Add node properties
		echo "node.environment=demo" >> "${SCRIPT_DIR}/presto-worker-$i/etc/node.properties"
		echo "node.id=${i}" >> "${SCRIPT_DIR}/presto-worker-$i/etc/node.properties"
...
set_coordinator_config_file() {
	...
	echo "discovery.uri=http://127.0.0.1:$COORD_PORT" >> "${SCRIPT_DIR}/presto-worker-coordinator/etc/config.properties"
		...
		for ((j = 0; j < $WORKERS_PER_DOMAIN; j += 1)) ; do
			for ((i = 0; i < $TOTAL_CORES; i += 1)) ; do
				...
				if [[ $i -eq 0 ]] && [[ $j -eq 0 ]]; then
					...
					w=$(($i+$j*$TOTAL_CORES))
					COMMAND="$COMMAND ${SCRIPT_DIR}/presto-worker-${w}/bin/launcher start"
					TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
					echo "${TSTAMP} ${COMMAND}"
					TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
					echo -n "${TSTAMP}"
...
run_tpch_benchmark() {
	...
	do
		...
		for ((i = 0; i < 10; i += 1)) ; do
			{ time ${SCRIPT_DIR}/presto --server localhost:$COORD_PORT --catalog tpch --schema sf$SCALE_FACTOR --file ${SCRIPT_DIR}/sql-tpch/q$q.sql ; } &>> $OUT_DIR/Q${q}_runtime.txt
		done
	done
}
