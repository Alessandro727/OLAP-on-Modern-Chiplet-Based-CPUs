...
#%
#% DESCRIPTION
#%    Script to run the TPC-H or JCC-H benchmark on SingleStore on multiple machines. 
#%    You can specify two different deployment strategies (WIM+FT or WIN+MEM).
#%
#% OPTIONS
...
#%    -a, --coordinator-address     Mandatory: coordinator-workerinstance IP address and port (format ip_address:port).
...
#%
#% EXAMPLES
#%    ${SCRIPT_NAME} --coordinator-worker yes -n 1 -a 192.168.10.10:8090 -l 192.168.10.10:8090 -m WIM -c 3 -s 10
#%
#================================================================
#-
#- IMPLEMENTATION
...
start_coordinator_and_workers() {
    ...
    if [[ $COORDINATOR_FLAG == 'yes' ]]; then
            memsql-toolbox-config register-host --host $LOCAL_IP -y
            memsql-admin create-node --host $LOCAL_IP --port 3306 --password '' --force-master --role MASTER --license $SSDB_LICENSE --yes
            sleep 15
    fi
    ...
    if [ ! -f "mlist.txt" ]; then
            touch mlist.txt
            echo "$LOCAL_IP:$LOCAL_PORT" >> mlist.txt
    fi


	while IFS=':' read -r col1_ip col2_port
	do 
	    memsql-toolbox-config register-host --host "$col1_ip" -y
		...
		if [[ ($MODE == 'WIN') ]]; then
					...
					if [[ $POLICY == "INT" ]]; then
						...
						COMMAND="${COMMAND} -m $(($i%$NUM_NUMA_NODES))"
                                        ...
                                        if [[ $COORDINATOR_FLAG == 'no' ]]; then
                                                COMMAND_1="$COMMAND memsql-admin create-node --host $col1_ip --port $cport --password '' --yes"
                                                TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
                                                ...
                                                if [[ $col1_ip == $LOCAL_IP ]]; then
                                                        ...
                                                        echo -n "${TSTAMP}"
                                                        $COMMAND_1
                                                        COMMAND_2="$COMMAND memsql-admin add-leaf --host $col1_ip --port $cport --password '' --yes"
                                                        TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
			...
			for ((i = 0; i < $TOTAL_WORKERS; i += 1)) ; do
				...
				if [[ $COORDINATOR_FLAG == 'no' ]]; then
					COMMAND_1="$COMMAND memsql-admin create-node --host $col1_ip --port $col2_port --password '' --yes"
					TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
					echo "${TSTAMP} ${COMMAND_1}"
					TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
					echo -n "${TSTAMP}"
					...
					if [[ $col1_ip == $LOCAL_IP ]]; then
						...
						echo -n "${TSTAMP}"
						$COMMAND_1
						COMMAND_2="$COMMAND memsql-admin add-leaf --host $col1_ip --port $col2_port --password '' --yes"
						TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
						echo "${TSTAMP} ${COMMAND_2}"
						TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
						echo -n "${TSTAMP}"
...
#============================
	...
	#== set short options ==#
SCRIPT_OPTS=':o:txhv:s:q:w:d:b:m:n:a:c:l:-:'
...
if [[ $COORDINATOR_FLAG == 'yes' ]]; then
	...
	infotitle "Wait for the Coordinator machine to run the benchmark and then press Ctrl+C..."

	( trap exit SIGINT ; read -r -d '' _ </dev/tty )

	memsql-toolbox-config unregister-host --all -y
