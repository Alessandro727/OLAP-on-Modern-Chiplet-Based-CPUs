...
do
	...
	CORES=$((${TOTAL_CORES}/${i}))
	cd ../
	#./Greenplum_benchmark.sh -t -s 100 -m WIM -w $i -c $CORES -d number_of_worker_instances/${i}_workers/
        ./Greenplum_benchmark.sh -t -s 100 -m WIC -c $i -d number_of_worker_instances/${i}_workers/
