...
start_coordinator_and_workers() {
	...
	WEB_PORT=$CANDIDATE
	if [[ ($MODE == 'WIN') ]]; then
		COMMAND="numactl --physcpubind=0-$(($TOTAL_CORES-1))"
		get_port
		COORD_PORT=$CANDIDATE
		COMMAND="$COMMAND ${SCRIPT_DIR}/spark-2.4.5-bin-hadoop2.7/bin/spark-class org.apache.spark.deploy.master.Master --ip 127.0.0.1 --port $COORD_PORT --webui-port $WEB_PORT &>> Master.log &"
		TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
		echo "${TSTAMP} ${COMMAND}"
		TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
		echo "${TSTAMP}"
		...
		for ((j = 0; j < $WORKERS_PER_DOMAIN; j += 1)) ; do
			...
			for ((i = 0; i < $NUM_NUMA_NODES; i += 1)) ; do
				...
				COMMAND="$COMMAND ${SCRIPT_DIR}/spark-2.4.5-bin-hadoop2.7/bin/spark-class org.apache.spark.deploy.worker.Worker spark://127.0.0.1:$COORD_PORT --webui-port $WORKER_PORT &>> Worker_$w.log &"
				TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
				echo "${TSTAMP} ${COMMAND}"
				TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
				#echo "${TSTAMP}"
		...
		get_port
		COORD_PORT=$CANDIDATE
		COMMAND="$COMMAND ./spark-2.4.5-bin-hadoop2.7/bin/spark-class org.apache.spark.deploy.master.Master --ip 127.0.0.1 --port $COORD_PORT --webui-port $WEB_PORT &>> Master.log &"
		TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
		echo "${TSTAMP} ${COMMAND}"
		TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
		echo "${TSTAMP}"
		...
		for ((i = 0; i < $TOTAL_WORKERS; i += 1)) ; do
			...
			get_port
			WORKER_PORT=$CANDIDATE
			COMMAND="$COMMAND ./spark-2.4.5-bin-hadoop2.7/bin/spark-class org.apache.spark.deploy.worker.Worker spark://127.0.0.1:$COORD_PORT --webui-port $WORKER_PORT &>> Worker_$(($i+1)).log &"
			TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
			echo "${TSTAMP} ${COMMAND}"
			TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
			#echo "${TSTAMP}"
		...
		for ((j = 0; j < $WORKERS_PER_DOMAIN; j += 1)) ; do
			for ((i = 0; i < $TOTAL_CORES; i += 1)) ; do
			  ...
			  #MEMB=$(($i%$NUM_NUMA_NODES))
				...
				#w=$(($i+$j*$TOTAL_CORES))
				w=$(($w+1))
				COMMAND="$COMMAND ./spark-2.4.5-bin-hadoop2.7/bin/spark-class org.apache.spark.deploy.worker.Worker spark://127.0.0.1:$COORD_PORT --webui-port $WORKER_PORT &>> Worker_$w.log &"
				TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
				echo "${TSTAMP} ${COMMAND}"
				TSTAMP="$( date ${SCRIPT_TIMELOG_FORMAT} ) ";
				#echo "${TSTAMP}"
...
run_tpch_benchmark() {
	...
	EXEC_MEMORY=$(($PHYMEM / $TOTAL_WORKERS))
	EXEC_MEMORY=$(echo "scale=0; ${EXEC_MEMORY} / 1.18" | bc)
	for q in $(echo $QUERY | sed "s/,/ /g")
	do
		print_message_with_date_and_newline "Running Query $q..."
		#for ((i = 0; i < 10; i += 1)) ; do
			#{ time ./presto --server localhost:$COORD_PORT --catalog tpch --schema sf$SCALE_FACTOR --file ./sql-tpch/q$q.sql ; } &>> $OUT_DIR/Q${q}_runtime.txt
			if [[ $BENCH == 'TPC-H' ]]; then
				{ time ${SCRIPT_DIR}/spark-2.4.5-bin-hadoop2.7/bin/spark-submit --class "main.scala.TpchQuery" --master spark://127.0.0.1:$COORD_PORT --executor-memory ${EXEC_MEMORY}K ${SCRIPT_DIR}/benchmark-sparksql/target/scala-2.11/spark-tpc-h-queries_2.11-1.0.jar "$q" ; } &>> Q$q.log
			else
				{ time ${SCRIPT_DIR}/spark-2.4.5-bin-hadoop2.7/bin/spark-submit --class "main.scala.JcchQuery" --master spark://127.0.0.1:$COORD_PORT --executor-memory ${EXEC_MEMORY}K ${SCRIPT_DIR}/benchmark-sparksql/target/scala-2.11/spark-tpc-h-queries_2.11-1.0.jar "$q" ; } &>> JQ$q.log
			fi
		#done
		value=$(printf "%02d" $q)
		if [[ $BENCH == 'TPC-H' ]]; then
...
if [[ $BENCH == 'TPC-H' ]]; then
	exec_cmd "mv TPCHQueriesExecTimes.txt $OUT_DIR"
	exec_cmd "mv Q* $OUT_DIR"
else
	exec_cmd "mv JCCHQueriesExecTimes.txt $OUT_DIR"
	exec_cmd "mv JQ* $OUT_DIR"
fi
