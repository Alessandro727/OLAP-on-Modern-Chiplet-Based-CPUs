...
do
        ...
        echo ""
        cd ..
        ./SparkSQL_benchmark.sh -t -s 100 -m WIM -w 1 -c $i -d multi-core_scalability/${i}_cores/
        cd multi-core_scalability/
done

cd ..
./SparkSQL_benchmark.sh -t -s 100 -m WIN -p MEM -d multi-core_scalability/WIN+MEM/
cd multi-core_scalability/
