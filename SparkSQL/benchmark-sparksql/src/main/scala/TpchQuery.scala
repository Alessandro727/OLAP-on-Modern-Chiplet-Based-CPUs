...
object TpchQuery {
  ...
  def executeQueries(sc: SparkContext, schemaProvider: TpchSchemaProvider, queryNum: Int): ListBuffer[(String, Float)] = {
    ...
    for (queryNo <- 1 to 10) {
      ...
      val query = Class.forName(f"main.scala.Q${queryNum}%02d").newInstance.asInstanceOf[TpchQuery]
  ...
  def main(args: Array[String]): Unit = {
    ...
    val scriptPath = System.getProperty("user.dir")
    val INPUT_DIR = "file:///"+scriptPath+"/benchmark-sparksql/dbgen"
    ...
    val outFile = new File("TPCHQueriesExecTimes.txt")
